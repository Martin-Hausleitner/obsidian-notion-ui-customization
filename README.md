# Obsidian Notion UI Customization

Clean Notion-style Obsidian workspace setup using real community plugins and local Markdown files.

## What Is Included

- Minimal theme CSS polish in `.obsidian/snippets/minimal-notion-workspace.css`
- Emoji-first navigation via Emoji Title and Iconic configuration
- Slash Commander configuration with 53 icon commands
- Bases, DB Folder, SQLSeal, and Dataview demo pages
- Company demo Home screen with collapsed details
- Small demo dataset for SQLSeal CSV queries
- Sanitized banner asset for the dashboard pages

## Not Included

- Personal vault content
- Notion credentials or sync state
- Obsidian Local REST API keys
- Plugin binaries or credentials
- Private screenshots

## Core Plugins And Community Plugins

Install the community plugins listed in `.obsidian/community-plugins.json`, then enable the CSS snippet:

1. Open Obsidian settings.
2. Go to Appearance, then CSS snippets.
3. Enable `minimal-notion-workspace`.
4. Restart Obsidian once after enabling SQLSeal, Slash Commander, Iconic, Emoji Title, Dataview, DB Folder, Copilot, Surfing, Notebook Navigator, Custom Frames, and Banners.

## Demo Pages

- `00_Dashboard/Home.md`
- `00_Dashboard/Data Tables.md`
- `05_Demo/GitHub Backup.md`
- `05_Demo/Knowledge Base.md`
- `05_Demo/Customer Portal.md`
- `05_Demo/Company Demo.base`
- `05_Demo/Company Demo DB.md`

## Safety

Do not add personal vault content, screenshots, workspace state, Notion sync state, Local REST API data, Copilot provider data, or plugin folders that contain tokens.
